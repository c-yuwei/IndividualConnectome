...
for h = 1 :length(nncv_tol)
    ...
    for i = 1:5
        ...
        for k = train_fold_indices
            ...
            for j = 1:length(dp_list)
                ...
                if ~isempty(idx)
                    ...
                    missing_subjects{end+1} = fold_subject_ids{j}; % Store full subject ID
                end
            end
            ...
            vois_dp_list_ID = cellfun(@(dp) dp.get('ID'), vois_dp_list, 'UniformOutput', false);


            if ~isequal(vois_dp_list_ID,fold_subject_ids)
                disp('voi and input subject not match')
                break
            end
        ...
        mdl = fitcknn(train_feature_values, train_target_classes, 'NumNeighbors', 5);
