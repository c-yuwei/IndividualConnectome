...
%% load Nifty images
%%group1
im_gr1_WM_GM = ImporterGroupSubjNIfTI('DIRECTORY',[fileparts(which('Example data Nifti diverse SUVR/atlas.xlsx')) filesep 'Group1'], ...
    'NIFTI_TYPE', {'T1'},...
    'WAITBAR', true);
gr1_WM_GM = im_gr1_WM_GM.get('GR');

im_gr1_PET = ImporterGroupSubjNIfTI('DIRECTORY',[fileparts(which('Example data Nifti diverse SUVR/atlas.xlsx')) filesep 'Group1'], ...
    'NIFTI_TYPE', {'PET'},...
    'WAITBAR', true);
gr1_PET = im_gr1_PET.get('GR');

%%group2
im_gr2_WM_GM = ImporterGroupSubjNIfTI('DIRECTORY',[fileparts(which('Example data Nifti diverse SUVR/atlas.xlsx')) filesep 'Group2'], ...
    'NIFTI_TYPE', {'T1'},...
    'WAITBAR', true);
gr2_WM_GM = im_gr2_WM_GM.get('GR');

im_gr2_PET = ImporterGroupSubjNIfTI('DIRECTORY',[fileparts(which('Example data Nifti diverse SUVR/atlas.xlsx')) filesep 'Group2'], ...
    'NIFTI_TYPE', {'PET'},...
    'WAITBAR', true);
...
for m = 1:numel(measures)
    ...
    % --- get PFBs ---
    pfb_dist = c_WU_SUVR.get('COMPARISON', meas).get('PFBG');
