...
gr_cn_dist = cnpos.get('GR_CON');

%% Mahalanobis connectivity classification
% gr_AD_dist  = AD+ Mahalanobis connectivity, reference = CN-
% gr_mci_dist = MCI+ Mahalanobis connectivity, reference = CN-
% gr_cn_dist  = CN+ Mahalanobis connectivity, reference = CN-

method_name = 'PET_SUVR_mahalanobis_connectivity';

%% Analyze connectivity groups as weighted undirected graphs

a_dist_ad = AnalyzeEnsemble_CON_WU( ...
    'GR', gr_AD_dist ...
    );

a_dist_mci = AnalyzeEnsemble_CON_WU( ...
    'TEMPLATE', a_dist_ad, ...
    'GR', gr_mci_dist ...
    );

a_dist_cn = AnalyzeEnsemble_CON_WU( ...
    'TEMPLATE', a_dist_ad, ...
    'GR', gr_cn_dist ...
    );

% Force graph calculation
a_dist_ad.memorize('G_DICT');
a_dist_mci.memorize('G_DICT');
a_dist_cn.memorize('G_DICT');

%% Convert graph groups to NN datasets

d_ad_dist = make_dataset_graph_CLA(a_dist_ad, 'AD');
d_mci_dist = make_dataset_graph_CLA(a_dist_mci, 'MCI');
d_cn_dist = make_dataset_graph_CLA(a_dist_cn, 'CN');
...
for h = 1:number_runs
    ...
    %% AD vs CN
    split_ad_cn = make_kfold_split_index({d_ad_dist, d_cn_dist}, kfolds, h);

    nncv_ad_cn = NNClassifierMLP_CrossValidation( ...
        'D', {d_ad_dist, d_cn_dist}, ...
        'KFOLDS', kfolds, ...
        'SPLIT', split_ad_cn, ...
    ...
    %% AD vs MCI
    split_ad_mci = make_kfold_split_index({d_ad_dist, d_mci_dist}, kfolds, h);

    nncv_ad_mci = NNClassifierMLP_CrossValidation( ...
        'D', {d_ad_dist, d_mci_dist}, ...
        'KFOLDS', kfolds, ...
        'SPLIT', split_ad_mci, ...
        'NN_TEMPLATE', nn_template_local ...
        );
    ...
    %% MCI vs CN
    split_mci_cn = make_kfold_split_index({d_mci_dist, d_cn_dist}, kfolds, h);

    nncv_mci_cn = NNClassifierMLP_CrossValidation( ...
        'D', {d_mci_dist, d_cn_dist}, ...
        'KFOLDS', kfolds, ...
        'SPLIT', split_mci_cn, ...
        'NN_TEMPLATE', nn_template_local ...
        );
...
out_dir = fullfile(pwd, ['mahalanobis_connectivity_classification_results']);

if ~exist(out_dir, 'dir')
    mkdir(out_dir)
end

save(fullfile(out_dir, 'PET_SUVR_mahalanobis_CON_CLA_results.mat'), ...
    'method_name', ...
    'number_runs', ...
    'kfolds', ...
    'ad_cn_auc', ...
...
writetable(result_table, fullfile(out_dir, 'PET_SUVR_mahalanobis_CON_CLA_summary.csv'));
