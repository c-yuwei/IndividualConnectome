classdef MeasureEnsemblePF_NU < MeasureEnsemblePF
	...
	methods (Static) % inspection
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = subclasses('MeasureEnsemblePF_NU', [], [], true);
		end
		...
		function prop_number = getPropNumber(varargin)
			...
			prop_number = numel(MeasureEnsemblePF_NU.getProps(varargin{:}));
		end
		function check_out = existsProp(prop)
			...
			check = any(prop == MeasureEnsemblePF_NU.getProps());
			
			if nargout == 1
				check_out = check;
			elseif ~check
				error( ...
					[BRAPH2.STR ':MeasureEnsemblePF_NU:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':MeasureEnsemblePF_NU:' BRAPH2.WRONG_INPUT '\n' ...
		...
		function check_out = existsTag(tag)
			...
			measureensemblepf_nu_tag_list = cellfun(@(x) MeasureEnsemblePF_NU.getPropTag(x), num2cell(MeasureEnsemblePF_NU.getProps()), 'UniformOutput', false);
			check = any(strcmp(tag, measureensemblepf_nu_tag_list));
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				measureensemblepf_nu_tag_list = cellfun(@(x) MeasureEnsemblePF_NU.getPropTag(x), num2cell(MeasureEnsemblePF_NU.getProps()), 'UniformOutput', false);
				prop = find(strcmp(pointer, measureensemblepf_nu_tag_list)); % tag = pointer
			else % numeric
				prop = pointer;
			end
		end
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				switch prop
					case MeasureEnsemblePF_NU.NODE
						tag = MeasureEnsemblePF_NU.NODE_TAG;
					case MeasureEnsemblePF_NU.LAYER
						tag = MeasureEnsemblePF_NU.LAYER_TAG;
					otherwise
		...
		function prop_category = getPropCategory(pointer)
			...
			switch prop
				case MeasureEnsemblePF_NU.NODE
					prop_category = MeasureEnsemblePF_NU.NODE_CATEGORY;
				case MeasureEnsemblePF_NU.LAYER
					prop_category = MeasureEnsemblePF_NU.LAYER_CATEGORY;
				otherwise
		...
		function prop_format = getPropFormat(pointer)
			...
			switch prop
				case MeasureEnsemblePF_NU.NODE
					prop_format = MeasureEnsemblePF_NU.NODE_FORMAT;
				case MeasureEnsemblePF_NU.LAYER
					prop_format = MeasureEnsemblePF_NU.LAYER_FORMAT;
				otherwise
		...
		function prop_description = getPropDescription(pointer)
			...
			switch prop
				...
				case MeasureEnsemblePF_NU.ELCLASS
					prop_description = 'ELCLASS (constant, string) is the class of the MeasureEnsemblePF_NU.';
				case MeasureEnsemblePF_NU.NAME
					prop_description = 'NAME (constant, string) is the name of the panel figure nodal unilayer group comparison figure.';
				case MeasureEnsemblePF_NU.DESCRIPTION
				...
				case MeasureEnsemblePF_NU.SETUP
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				...
				case MeasureEnsemblePF_NU.TEMPLATE
					prop_default = Format.getFormatDefault(Format.ITEM, MeasureEnsemblePF_NU.getPropSettings(prop));
				case MeasureEnsemblePF_NU.ID
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			%  Error id: €BRAPH2.STR€:MeasureEnsemblePF_NU:€BRAPH2.WRONG_INPUT€
			% 
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			...
			switch prop
				case MeasureEnsemblePF_NU.NODE % __MeasureEnsemblePF_NU.NODE__
					check = Format.checkFormat(Format.SCALAR, value, MeasureEnsemblePF_NU.getPropSettings(prop));
				case MeasureEnsemblePF_NU.LAYER % __MeasureEnsemblePF_NU.LAYER__
					check = Format.checkFormat(Format.SCALAR, value, MeasureEnsemblePF_NU.getPropSettings(prop));
				case MeasureEnsemblePF_NU.TEMPLATE % __MeasureEnsemblePF_NU.TEMPLATE__
					check = Format.checkFormat(Format.ITEM, value, MeasureEnsemblePF_NU.getPropSettings(prop));
				otherwise
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(pf, prop, varargin)
			...
			switch prop
				case MeasureEnsemblePF_NU.SETUP % __MeasureEnsemblePF_NU.SETUP__
					me = pf.get('ME');
