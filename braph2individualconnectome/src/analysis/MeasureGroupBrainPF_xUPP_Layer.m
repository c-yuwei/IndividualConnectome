classdef MeasureGroupBrainPF_xUPP_Layer < PanelProp
	...
	methods (Static) % inspection
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = subclasses('MeasureGroupBrainPF_xUPP_Layer', [], [], true);
		end
		function prop_list = getProps(category)
			...
			if nargin == 0
				prop_list = [ ...
					PanelProp.getProps() ...
						MeasureGroupBrainPF_xUPP_Layer.DROPDOWN ...
						];
				return
			end
		...
		function prop_number = getPropNumber(varargin)
			...
			prop_number = numel(MeasureGroupBrainPF_xUPP_Layer.getProps(varargin{:}));
		end
		function check_out = existsProp(prop)
			...
			check = any(prop == MeasureGroupBrainPF_xUPP_Layer.getProps());
			
			if nargout == 1
				check_out = check;
			elseif ~check
				error( ...
					[BRAPH2.STR ':MeasureGroupBrainPF_xUPP_Layer:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':MeasureGroupBrainPF_xUPP_Layer:' BRAPH2.WRONG_INPUT '\n' ...
		...
		function check_out = existsTag(tag)
			...
			measuregroupbrainpf_xupp_layer_tag_list = cellfun(@(x) MeasureGroupBrainPF_xUPP_Layer.getPropTag(x), num2cell(MeasureGroupBrainPF_xUPP_Layer.getProps()), 'UniformOutput', false);
			check = any(strcmp(tag, measuregroupbrainpf_xupp_layer_tag_list));
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				measuregroupbrainpf_xupp_layer_tag_list = cellfun(@(x) MeasureGroupBrainPF_xUPP_Layer.getPropTag(x), num2cell(MeasureGroupBrainPF_xUPP_Layer.getProps()), 'UniformOutput', false);
				prop = find(strcmp(pointer, measuregroupbrainpf_xupp_layer_tag_list)); % tag = pointer
			else % numeric
				prop = pointer;
			end
		end
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				switch prop
					case MeasureGroupBrainPF_xUPP_Layer.DROPDOWN
						tag = MeasureGroupBrainPF_xUPP_Layer.DROPDOWN_TAG;
					otherwise
		...
		function prop_category = getPropCategory(pointer)
			...
			switch prop
				case MeasureGroupBrainPF_xUPP_Layer.DROPDOWN
					prop_category = MeasureGroupBrainPF_xUPP_Layer.DROPDOWN_CATEGORY;
				otherwise
		...
		function prop_format = getPropFormat(pointer)
			...
			switch prop
				case MeasureGroupBrainPF_xUPP_Layer.DROPDOWN
					prop_format = MeasureGroupBrainPF_xUPP_Layer.DROPDOWN_FORMAT;
				otherwise
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				case MeasureGroupBrainPF_xUPP_Layer.DROPDOWN
					prop_default = Format.getFormatDefault(Format.HANDLE, MeasureGroupBrainPF_xUPP_Layer.getPropSettings(prop));
				case MeasureGroupBrainPF_xUPP_Layer.ELCLASS
				...
				case MeasureGroupBrainPF_xUPP_Layer.TEMPLATE
					prop_default = Format.getFormatDefault(Format.ITEM, MeasureGroupBrainPF_xUPP_Layer.getPropSettings(prop));
				case MeasureGroupBrainPF_xUPP_Layer.ID
				...
				case MeasureGroupBrainPF_xUPP_Layer.PROP
					prop_default = MeasureGroupBrainPF_NU.LAYER;
				case MeasureGroupBrainPF_xUPP_Layer.HEIGHT
					prop_default = s(4);
				otherwise
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			%  Error id: €BRAPH2.STR€:MeasureGroupBrainPF_xUPP_Layer:€BRAPH2.WRONG_INPUT€
			% 
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			...
			switch prop
				case MeasureGroupBrainPF_xUPP_Layer.DROPDOWN % __MeasureGroupBrainPF_xUPP_Layer.DROPDOWN__
					check = Format.checkFormat(Format.HANDLE, value, MeasureGroupBrainPF_xUPP_Layer.getPropSettings(prop));
				case MeasureGroupBrainPF_xUPP_Layer.TEMPLATE % __MeasureGroupBrainPF_xUPP_Layer.TEMPLATE__
					check = Format.checkFormat(Format.ITEM, value, MeasureGroupBrainPF_xUPP_Layer.getPropSettings(prop));
				otherwise
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(pr, prop, varargin)
			...
			switch prop
				...
				case MeasureGroupBrainPF_xUPP_Layer.X_DRAW % __MeasureGroupBrainPF_xUPP_Layer.X_DRAW__
					value = calculateValue@PanelProp(pr, PanelProp.X_DRAW, varargin{:}); % also warning
					if value
					    pr.memorize('DROPDOWN')
					end
					
				case MeasureGroupBrainPF_xUPP_Layer.UPDATE % __MeasureGroupBrainPF_xUPP_Layer.UPDATE__
				...
				case MeasureGroupBrainPF_xUPP_Layer.REDRAW % __MeasureGroupBrainPF_xUPP_Layer.REDRAW__
				...
				case MeasureGroupBrainPF_xUPP_Layer.DELETE % __MeasureGroupBrainPF_xUPP_Layer.DELETE__
