classdef MeasureEnsembleBrainPF_BB < MeasureEnsembleBrainPF
	...
	methods (Static) % inspection
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = subclasses('MeasureEnsembleBrainPF_BB', [], [], true);
		end
		...
		function prop_number = getPropNumber(varargin)
			...
			prop_number = numel(MeasureEnsembleBrainPF_BB.getProps(varargin{:}));
		end
		function check_out = existsProp(prop)
			...
			check = any(prop == MeasureEnsembleBrainPF_BB.getProps());
			
			if nargout == 1
				check_out = check;
			elseif ~check
				error( ...
					[BRAPH2.STR ':MeasureEnsembleBrainPF_BB:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':MeasureEnsembleBrainPF_BB:' BRAPH2.WRONG_INPUT '\n' ...
		...
		function check_out = existsTag(tag)
			...
			measureensemblebrainpf_bb_tag_list = cellfun(@(x) MeasureEnsembleBrainPF_BB.getPropTag(x), num2cell(MeasureEnsembleBrainPF_BB.getProps()), 'UniformOutput', false);
			check = any(strcmp(tag, measureensemblebrainpf_bb_tag_list));
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				measureensemblebrainpf_bb_tag_list = cellfun(@(x) MeasureEnsembleBrainPF_BB.getPropTag(x), num2cell(MeasureEnsembleBrainPF_BB.getProps()), 'UniformOutput', false);
				prop = find(strcmp(pointer, measureensemblebrainpf_bb_tag_list)); % tag = pointer
			else % numeric
				prop = pointer;
			end
		end
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				...
				case MeasureEnsembleBrainPF_BB.TEMPLATE
					prop_default = Format.getFormatDefault(Format.ITEM, MeasureEnsembleBrainPF_BB.getPropSettings(prop));
				case MeasureEnsembleBrainPF_BB.ID
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			%  Error id: €BRAPH2.STR€:MeasureEnsembleBrainPF_BB:€BRAPH2.WRONG_INPUT€
			% 
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			...
			switch prop
				case MeasureEnsembleBrainPF_BB.TEMPLATE % __MeasureEnsembleBrainPF_BB.TEMPLATE__
					check = Format.checkFormat(Format.ITEM, value, MeasureEnsembleBrainPF_BB.getPropSettings(prop));
				otherwise
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(pf, prop, varargin)
			...
			switch prop
				case MeasureEnsembleBrainPF_BB.SETUP % __MeasureEnsembleBrainPF_BB.SETUP__
					%%%__WARN_TBI__
					value = [];
