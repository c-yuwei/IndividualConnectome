classdef MeasureGroupBrainPF_GB < MeasureGroupBrainPF
	...
	methods (Static) % inspection
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = subclasses('MeasureGroupBrainPF_GB', [], [], true);
		end
		...
		function prop_number = getPropNumber(varargin)
			...
			prop_number = numel(MeasureGroupBrainPF_GB.getProps(varargin{:}));
		end
		function check_out = existsProp(prop)
			...
			check = any(prop == MeasureGroupBrainPF_GB.getProps());
			
			if nargout == 1
				check_out = check;
			elseif ~check
				error( ...
					[BRAPH2.STR ':MeasureGroupBrainPF_GB:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':MeasureGroupBrainPF_GB:' BRAPH2.WRONG_INPUT '\n' ...
		...
		function check_out = existsTag(tag)
			...
			measuregroupbrainpf_gb_tag_list = cellfun(@(x) MeasureGroupBrainPF_GB.getPropTag(x), num2cell(MeasureGroupBrainPF_GB.getProps()), 'UniformOutput', false);
			check = any(strcmp(tag, measuregroupbrainpf_gb_tag_list));
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				measuregroupbrainpf_gb_tag_list = cellfun(@(x) MeasureGroupBrainPF_GB.getPropTag(x), num2cell(MeasureGroupBrainPF_GB.getProps()), 'UniformOutput', false);
				prop = find(strcmp(pointer, measuregroupbrainpf_gb_tag_list)); % tag = pointer
			else % numeric
				prop = pointer;
			end
		end
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				...
				case MeasureGroupBrainPF_GB.TEMPLATE
					prop_default = Format.getFormatDefault(Format.ITEM, MeasureGroupBrainPF_GB.getPropSettings(prop));
				case MeasureGroupBrainPF_GB.ID
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			%  Error id: €BRAPH2.STR€:MeasureGroupBrainPF_GB:€BRAPH2.WRONG_INPUT€
			% 
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			...
			switch prop
				case MeasureGroupBrainPF_GB.TEMPLATE % __MeasureGroupBrainPF_GB.TEMPLATE__
					check = Format.checkFormat(Format.ITEM, value, MeasureGroupBrainPF_GB.getPropSettings(prop));
				otherwise
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(pf, prop, varargin)
			...
			switch prop
				case MeasureGroupBrainPF_GB.SETUP % __MeasureGroupBrainPF_GB.SETUP__
					%%%__WARN_TBI__
					value = [];
