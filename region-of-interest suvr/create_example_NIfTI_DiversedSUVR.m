function create_example_NIfTI(atlas_path, output_dir, group_names, num_files_per_group, seed)
    ...
    if nargin < 2 || isempty(output_dir)
        output_dir = fullfile(fileparts(which('SubjectNIfTI')), 'Example data Nifti diverse SUVR');
    end
    ...
    for group_idx = 1:length(group_names)
        ...
        for file_idx = 1:num_files_per_group
            ...
            if ~isempty(special_idxs)
                ...
                roi_mean_base = 10;  % Example base mean
                roi_std_base = 3;    % Example base std

                % Cov matrix: diagonal variance, off-diagonal cov for Group2
                cov_matrix = eye(n_special) * roi_std_base^2;
                if group_idx == 2
                    cov_matrix = cov_matrix + ones(n_special) * 5;  % Add cov (adjust strength)
                    cov_matrix(1:n_special+1:end) = roi_std_base^2;  % Restore diag (MATLAB equiv of np.fill_diagonal)
                end

                special_means = mvnrnd(roi_mean_base * ones(1, n_special), cov_matrix);

                % Assign to means_all_regions (for VOIs and sampling)
                means_all_regions(special_idxs) = special_means;
                stds_all_regions(special_idxs)  = roi_std_base;
            end
            ...
            for region_idx = 1:num_regions
                ...
                % Use means_all_regions(region_idx) as mean_val for sampling
                mean_val = means_all_regions(region_idx);
                std_val = stds_all_regions(region_idx);  % Or fixed voxel_std = 1 for all
