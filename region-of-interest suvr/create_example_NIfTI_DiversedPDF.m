function create_example_NIfTI(atlas_path, output_dir, group_names, num_files_per_group, seed)
    ...
    if nargin < 2 || isempty(output_dir)
        output_dir = fullfile(fileparts(which('SubjectNIfTI')), 'Example data Nifti diverse');
    end
    ...
    % Precompute shared means and stds for special regions (21-40) across both groups
    special_start = 21;
    special_end = 40;
    special_range = special_start:special_end;
    num_special = length(special_range);
    shared_special_means = 15;%abs((rand(num_files_per_group, num_special) + 1) * 10);  % Per subject/file_idx, per special region
    shared_special_stds  = 5;%abs(1 + rand(num_files_per_group, num_special) * 9);
        ...
        for file_idx = 1:num_files_per_group
            ...
            for region_idx = 1:num_regions
                ...
                if ismember(region_idx, special_range)
                    % Regions 21-40: SAME mean/std for BOTH groups (use precomputed per file_idx)
                    special_idx = region_idx - special_start + 1;
                    mean_val = shared_special_means;%shared_special_means(file_idx, special_idx);
                    std_val  = shared_special_stds;%shared_special_stds(file_idx, special_idx);

                    if group_idx == 1
                        % Group1: Standard Gaussian
                        simulated_values = mean_val + std_val * randn(nnz(atlas_data == label), 1);
                    else
                        % Group2: Bimodal Gaussian mixture (multi-shaped) with same mean/variance
                        % Two components, equal weight 0.5
                        small_std = std_val * 0.3;  % Component std (tunable)
                        delta = sqrt(std_val^2 - small_std^2);  % Separation to match total variance
                        ...
                        simulated_values(comp_choice)  = (mean_val - delta) + small_std * randn(sum(comp_choice), 1);
                        simulated_values(~comp_choice) = (mean_val + delta) + small_std * randn(sum(~comp_choice), 1);
                    end
                elseif group_idx == 1 && region_idx <= 20
                    % Group1 first 20 regions: high shared mean/std
                    mean_val = shared_random_variable_mean * 10;
                    std_val  = shared_random_variable_std * 5;
                    simulated_values = mean_val + std_val * randn(nnz(atlas_data == label), 1);
                else
                    % All other cases: independent normal
                    mean_val = independent_means(region_idx);
                    std_val  = independent_stds(region_idx);
                    simulated_values = mean_val + std_val * randn(nnz(atlas_data == label), 1);
                end
                ...
                % Store statistics (intended parameters)
