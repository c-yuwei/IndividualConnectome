function create_data_NIfTI_PET_PDFShape(atlas_path, output_dir, group_names, num_files_per_group, seed, covarying_roi_indices)
% CREATE_DATA_NIFTI_PET_PDFSHAPE
% Generate PET-specific simulated NIfTI data with ROI-level PDF-shape covariation.
%
% Inputs:
...
%   - sub-XXXX/ses-01/pet/sub-XXXX_ses-01_pet.nii
    ...
    if nargin < 1 || isempty(atlas_path)
        atlas_path = fullfile( ...
            fileparts(which('create_data_NIfTI_PET_PDFShape')), ...
            'example atlases NIfTI', ...
            'aal120_atlas.nii' ...
            );
    end
    if nargin < 2 || isempty(output_dir)
        output_dir = fullfile(fileparts(which('create_data_NIfTI_PET_PDFShape')), 'Example data NIfTI PDF shape covariation');
    end
    ...
    if ~isempty(existing_pet_files)
        fprintf('PET NIfTI files already exist. Skipping PET PDF-shape data generation in: %s\n', output_dir);
        return
    end
    ...
    % Fixed target SUVR mean for all non-reference ROIs
    target_suvr_mean = 1.10;

    % Reference-region SUVR-like distribution
    reference_suvr_mean = 1.00;
    ...
    max_shape_delta = 0.50;

    % PET values are kept in a plausible SUVR-like range
    suvr_lower_bound = 0;
    suvr_upper_bound = 3;
    ...
    for group_idx = 1:numel(group_names)
        ...
        for file_idx = 1:num_files_per_group
            ...
            % Initialize subject data in SUVR-like space
            ...
            %% Generate ROI-specific PET SUVR-like values
            for region_idx = 1:num_regions
                ...
                simulated_data(region_mask) = single(roi_values);
                realized_means_all_regions(region_idx) = mean(roi_values);
            end
