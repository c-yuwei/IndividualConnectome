...
a_WU4 = AnalyzeEnsemble_FUN_WU( ...
    'GR', pdf_gr4, ...
    'REPETITION', 0, ...
    'GRAPH_TEMPLATE', g_temp...
);
gr4_corr = a_WU4.get('G_DICT');
...
CON_CN_id =  cellfun(@(sub) sub.get('ID'), gr1_corr.get('IT_LIST'), 'UniformOutput', false); 
...
CON_MCI_id =  cellfun(@(sub) sub.get('ID'), gr2_corr.get('IT_LIST'), 'UniformOutput', false); 
...
CON_ad_id =  cellfun(@(sub) sub.get('ID'), gr3_corr.get('IT_LIST'), 'UniformOutput', false); 
...
CON_CNpos_id =  cellfun(@(sub) sub.get('ID'), gr4_corr.get('IT_LIST'), 'UniformOutput', false); 
...
for i = 1:length(sub_list_CN)
    A = gr1_corr.get('IT',i);
    connectome_matrix = A.get('A');
    connectome_matrix = connectome_matrix{1};
    suvr_vector = suvr_CN{i};
    CON_CN_id = extractAfter(A.get('ID'), 'g ');
    if  isequal(CON_CN_id,suvr_CN_id{i})
        ...
        disp(fprintf('CON_CNpos_id{%d} is not same as suvr_CNpos_id{%d}',CON_CN_id, suvr_CN_id{i}))
    end
end
...
for i = 1:length(sub_list_MCI)
    ...
    baseline_matrix = repmat(suvr_vector', size(connectome_matrix,2),1); % Outer product for baseline layer
    CON_MCI_id = extractAfter(A.get('ID'), 'g ');
    if  isequal(CON_MCI_id,suvr_MCI_id{i})
        ...
        disp(fprintf('CON_CNpos_id{%d} is not same as suvr_CNpos_id{%d}',CON_MCI_id, suvr_MCI_id{i}))
    end
end
...
for i = 1:length(sub_list_AD)
    ...
    baseline_matrix = repmat(suvr_vector', size(connectome_matrix,2),1); % Outer product for baseline layer
    CON_AD_id = extractAfter(A.get('ID'), 'g ');
    if  isequal(CON_AD_id,suvr_ad_id{i})
        ...
        disp(fprintf('CON_CNpos_id{%d} is not same as suvr_CNpos_id{%d}',CON_AD_id, suvr_ad_id{i}))
    end
end
