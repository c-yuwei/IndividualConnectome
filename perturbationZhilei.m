...
OutputPath = [ pwd, filesep, 'PerturbationZhilei' ];
mkdir( OutputPath );
Tau = load(which('Tau.mat'));
Tau.Tau.Tau = [ Tau.Tau.SUVR_LH, Tau.Tau.SUVR_RH ];
Tau = Tau.Tau;
...
IndividualTauConnectome_AP = zeros( numel( IncludedScan_AP ),size( ReferenceTauConnectome,1), size( ReferenceTauConnectome,1));
for Scan = 1:numel( IncludedScan_AP )
    ...
    IndividualTauConnectome_AP( Scan, :, : ) =  (PerturbedTauConnectome - ReferenceTauConnectome) ./ (1 - ReferenceTauConnectome.^2 )*( numel( IncludedScan_Reference ) - 1 );
end
...
IndividualTauConnectome_NC_AP_first   = IndividualTauConnectome_NC_AP( ScanOrder_NC_AP == 1, :, : );
IndividualTauConnectome_MCI_AP_first   = IndividualTauConnectome_MCI_AP( ScanOrder_MCI_AP == 1, :, : );
IndividualTauConnectome_AD_AP_first   = IndividualTauConnectome_AD_AP( ScanOrder_AD_AP == 1, :, : );

IncludedScan_NC_AP_first   = IncludedScan_NC_AP( ScanOrder_NC_AP == 1, :, : );
IncludedScan_MCI_AP_first   = IncludedScan_MCI_AP( ScanOrder_MCI_AP == 1, :, : );
IncludedScan_AD_AP_first   = IncludedScan_AD_AP( ScanOrder_AD_AP == 1, :, : );

RID_first_NC_AP = Tau.RID(IncludedScan_NC_AP_first);
RID_first_MCI_AP = Tau.RID(IncludedScan_MCI_AP_first);
RID_first_AD_AP = Tau.RID(IncludedScan_AD_AP_first);
...
save( [ OutputPath, filesep, 'IndividualTauConnectome_NC_AP.mat' ], 'IncludedScan_NC_AP', 'IndividualTauConnectome_NC_AP', 'ScanOrder_NC_AP', 'TimeLag_NC_AP', 'IndividualTauConnectome_NC_AP_first', 'RID_first_NC_AP' );
save( [ OutputPath, filesep, 'IndividualTauConnectome_MCI_AP.mat' ], 'IncludedScan_MCI_AP', 'IndividualTauConnectome_MCI_AP', 'ScanOrder_MCI_AP', 'TimeLag_MCI_AP', 'IndividualTauConnectome_MCI_AP_first','RID_first_MCI_AP' );
save( [ OutputPath, filesep, 'IndividualTauConnectome_AD_AP.mat' ], 'IncludedScan_AD_AP', 'IndividualTauConnectome_AD_AP', 'ScanOrder_AD_AP', 'TimeLag_AD_AP', 'IndividualTauConnectome_AD_AP_first','RID_first_AD_AP');
